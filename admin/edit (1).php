...
if($task == "doedit") {
  ...
  $hours = str_replace( "'", "\\'", str_replace( "\\", "\\\\", $_POST['hours'] ) );
  $wifi = str_replace( "'", "\\'", str_replace( "\\", "\\\\", $_POST['wifi'] ) );
  $owner_email = $_POST['owner_email'];
  $lat = (float) $_POST['lat'];
  $lng = (float) $_POST['lng'];
  
  mysql_query("UPDATE places SET title='$title', type='$type', address='$address', uri='$uri', lat='$lat', lng='$lng', description='$description', owner_name='$owner_name', hours='$hours', wifi='$wifi', owner_email='$owner_email' WHERE id='$place_id' LIMIT 1") or die(mysql_error());
...
<form id="admin" class="form-horizontal" action="edit.php" method="post">
  ...
  <fieldset>
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <select class="input input-xlarge" name="type">
          ...
          <option<? if($place[type] == "restaurants") {?> selected="selected"<? } ?>>restaurants</option>
        </select>
      </div>
    </div>
    <div class="control-group">
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <textarea class="input input-xlarge" name="hours"><?=$place[hours]?></textarea>
      </div>
    </div>
    <div class="control-group">
      <label class="control-label" for="">Wifi</label>
      <div class="controls">
        <textarea class="input input-xlarge" name="wifi"><?=$place[wifi]?></textarea>
      </div>
    </div>
    <div class="control-group">
