...
$hours = mysql_real_escape_string(parseInput($_POST['hours']));
$wifi = mysql_real_escape_string(parseInput($_POST['wifi']));

// validate fields
if(empty($title) || empty($type) || empty($address) || empty($uri) || empty($description) || empty($hours) || empty($owner_name) || empty($wifi) || empty($owner_email)) {
  ...
  // normal mode enabled, save new data to local db
    ...
    $insert = mysql_query("INSERT INTO places (approved, title, type, address, uri, description, hours, wifi, owner_name, owner_email) VALUES (null, '$title', '$type', '$address', '$uri', '$description', '$hours', '$wifi', '$owner_name', '$owner_email')") or die(mysql_error());
